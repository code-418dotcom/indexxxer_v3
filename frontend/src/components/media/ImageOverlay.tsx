...
import { ExternalLink, Heart, ImageIcon, Sparkles, Tag, X } from "lucide-react";
...
export function ImageOverlay({ item, onClose, onSelectItem }: ImageOverlayProps) {
              ...
              <div className="grid grid-cols-3 gap-2">
                {similar.map((s) => (
                  <button
                    ...
                    className="relative aspect-square rounded-lg overflow-hidden bg-[var(--color-muted)] hover:ring-2 hover:ring-[hsl(217_91%_60%)] transition-all group"
                    title={s.filename}
                  >
                    {s.thumbnail_url ? (
                      <Image
